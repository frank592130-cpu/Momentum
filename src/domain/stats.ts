...
import { AnalyticsData, Goal, GoalDifficulty, GoalMetrics, RiskLevel, Task, WorkloadSlice } from "./models";
...
function getGoalTasks(tasks: Task[], goalId: string) {
...
function getRisk(successRate: number, paceScore: number): RiskLevel {
  if (successRate >= 80 && paceScore >= 80) return "low";
...
export function calculateGoalMetrics(goal: Goal, tasks: Task[], todayKey = toDateKey()): GoalMetrics {
  ...
  const linkedTasks = getGoalTasks(tasks, goal.id);
  ...
  const rawSuccessRate = Math.round(paceScore * 0.35 + effortScore * 0.35 + planningScore * 0.15 + momentumScore * 0.15);
  const daysLeft = diffInDays(todayKey, goal.deadline);
  const successRate = progress >= 100 ? 100 : applyDeadlineCap(rawSuccessRate, daysLeft, totalDays, progress, expectedProgress);
  ...
  return {
    ...
    risk: getRisk(successRate, paceScore),
...
function getActiveDays(tasks: Task[], days: string[]) {
  return days.filter((dateKey) => getTasksForDate(tasks, dateKey).some((task) => task.done)).length;
}

function getStreakDays(tasks: Task[], todayKey: string) {
  let streak = 0;
...
export function getAnalyticsData(goals: Goal[], tasks: Task[], todayKey = toDateKey()): AnalyticsData {
  ...
  const enrichedGoals = enrichGoals(goals, tasks, todayKey);
  const avgGoalSuccessRate = enrichedGoals.length
    ? Math.round(enrichedGoals.reduce((sum, goal) => sum + goal.successRate, 0) / enrichedGoals.length)
    : 0;

  const weeklyCompletion = last7.map((dateKey) => getCompletionRate(getTasksForDate(tasks, dateKey)));
  const focusHours = last7.map((dateKey) => getCompletedFocusHours(getTasksForDate(tasks, dateKey)));
  const successTrend = last7.map((dateKey) => {
    const goalsAtDate = enrichGoals(goals, tasks, dateKey);
    return goalsAtDate.length
      ? Math.round(goalsAtDate.reduce((sum, goal) => sum + goal.successRate, 0) / goalsAtDate.length)
      : 0;
  });

  const recentTasks = getTasksBetween(tasks, addDays(todayKey, -6), todayKey);
  const thisWeekAverageFocus = focusHours.reduce((sum, value) => sum + value, 0) / focusHours.length;
  const lastWeekAverageFocus = previous7
    .map((dateKey) => getCompletedFocusHours(getTasksForDate(tasks, dateKey)))
    .reduce((sum, value) => sum + value, 0) / previous7.length;

  return {
    successTrend,
    weeklyCompletion,
    focusHours,
    workloadDist: getWorkloadDistribution(recentTasks),
    avgGoalSuccessRate,
    weeklyCompletionRate: Math.round(weeklyCompletion.reduce((sum, value) => sum + value, 0) / weeklyCompletion.length),
    avgDailyFocusHours: round(thisWeekAverageFocus, 1),
    activeDays30: getActiveDays(tasks, last30),
    streakDays: getStreakDays(tasks, todayKey),
    bestWeekLabel: getBestWeekLabel(tasks, todayKey),
  };
}
...
export function getAnalyticsTrends(goals: Goal[], tasks: Task[], todayKey = toDateKey()) {
  ...
  const previous = {
    ...
    success:
      last7
        .map((dateKey) => {
          const previousDate = addDays(dateKey, -7);
          const goalsAtDate = enrichGoals(goals, tasks, previousDate);
          return goalsAtDate.length
            ? goalsAtDate.reduce((sum, goal) => sum + goal.successRate, 0) / goalsAtDate.length
            : 0;
        })
        .reduce((sum, value) => sum + value, 0) / last7.length,
  };

  return {
    success: getTrendPercent(current.avgGoalSuccessRate, previous.success),
    completion: getTrendPercent(current.weeklyCompletionRate, previous.completion),
    focus: getTrendPercent(current.avgDailyFocusHours, previous.focus),
  };
}
